...
class YAMLDijkstraNode:
    def __init__(self):
        ...
        # Publisher (latch zodat late abonnees het laatste bericht ook krijgen)
        ...
        rospy.loginfo(f"Planned path: {self.path_string}")
        rospy.loginfo(f"Path length: {len(path)} nodes")
        # Publiceer de route HERHAALD (elke seconde), zodat de lokalisatie-node
        # de route altijd ontvangt, ongeacht de opstartvolgorde. Dit lost een
        # timing-probleem op waarbij een eenmalig latch-bericht gemist werd.
        self.publish_loop()
    def publish_loop(self):
        rate = rospy.Rate(1)  # 1 Hz
        while not rospy.is_shutdown():
            self.pub_path.publish(self.path_string)
            rate.sleep()
